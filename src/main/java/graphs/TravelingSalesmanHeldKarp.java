...
public class TravelingSalesmanHeldKarp {
    ...
    private void generateCombination(int input[], int pos, int level, List<Set<Integer>> allSets, int result[]) {
        if(level == input.length) {
            return;
        }
        Set<Integer> set = createSet(result, level);
        allSets.add(set);
        for(int i=pos; i < input.length; i++) {
            result[level] = input[i];
            generateCombination(input, i+1, level+1, allSets, result);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        tsp.minCost(distance);
        tsp.generateCombination_1(new int [] {1, 2, 3});
    }

    public void generateCombination_1(int[] input) {
        int output[] = new int[input.length];
        boolean visited[] = new boolean[input.length];
        generatePermutation(input, 0, output, visited);
    }

    public void generateCombination_2(int input[], int pos, int level, int output[], boolean[] visited) {
        if(level == input.length || pos == input.length) {
            return;
        }
        for(int i=pos; i < input.length; i++) {
            if (!visited[i]) {
                ...
                visited[i] = true;
                printCombination(level, output);
                generateCombination_2(input, i + 1, level + 1, output, visited);
                visited[i] = false;
            }
        }
    }

    public void generatePermutation(int input[], int level, int output[], boolean[] visited) {
        if(level == input.length) {
            printCombination(2, output);
            return;
        }
        for(int i=0; i < input.length; i++) {
            if (!visited[i]) {
                output[level] = input[i];
                visited[i] = true;
                generatePermutation(input, level + 1, output, visited);
                visited[i] = false;
            }
        }
    }

    private void printCombination(int level, int[] output) {
